...
import 'package:grammar_app/models/hear_quizz.dart';

class HearPage extends StatefulWidget {
  @override
  _HearPage createState() => _HearPage();
}

class _HearPage extends State<HearPage> {
  List<Options> options = new List<Options>();
  List<HearModel> hearQuizz = new List<HearModel>();
  ...
  Future<List<String>> _loadSuggest() async {
    List<String> _listwords = [];
  ...
  void _setup() async {
    List<String> _listwords = await _loadSuggest();
    setState(() {
      this._listwords = _listwords;
    });
    Random rnd = new Random();
    int range = 0 + rnd.nextInt(5126 - 0);
    int correctAns = 0 + rnd.nextInt(4 - 0);
    List<String> _listOptions = _listwords.sublist(range, range + 4);
    options
    ...
    hearQuizz.add(new HearModel(_listOptions[correctAns], options, correctAns));
    await flutterTts.setLanguage("en-US");
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.setPitch(0.9);
    await flutterTts.speak(hearQuizz[0].question);
  }
                ...
                child: _index == 10
                    ? (_isAnswered
                        ...
                        : RaisedButton(
                            ...
                            onPressed: () {
                              setState(() {
                                if (hearQuizz[0].correctAns == _selectedAns) {
                                  _correctAns++;
                                ...
                                options[hearQuizz[0].correctAns].isSelected =
                                    true;
                                options[hearQuizz[0].correctAns].isAnswered =
                                    true;
                                _isAnswered = true;
                              });
                            },
                            ...
                            onPressed: () async {
                              setState(() {
                                ...
                                List<String> _listOptions =
                                    _listwords.sublist(range, range + 4);
                                ...
                                hearQuizz[0] = new HearModel(
                                    _listOptions[correctAns],
                                    options,
                                    correctAns);
                                _selectedAns = null;
                              });
